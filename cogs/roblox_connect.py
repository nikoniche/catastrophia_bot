...
class RobloxConnect(commands.Cog):
            ...
            # links the discord account to the roblox username if the status is 1
                ...
                if status == 1 or status == 3 or status == 4:
                    if status == 1:
                        ...
                        print("ATTEMPT TO LINK TO A MINORS ACCOUNT")
                        await channel.send(
                            f"{user.mention}" + "\n" + embed_message(
    ...
    @app_commands.command(
        name="link",
        description=
        "Initiates the process of linking your discord account to your roblox username."
    )
    ...
    async def removelink(self, interaction: discord.Interaction) -> None:
