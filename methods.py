...
CATASTROPHIA_API_URL = get_secret("TEST_URL")
...
async def error_message(bot, own_message: str, exception: Exception, response_text=None):
    ...
    filtered_exception = str(exception).replace(CATASTROPHIA_API_URL, "").replace(HIDDEN_URL, "")

    await error_channel.send("<@" + str(PING_ACCOUNT_ID) + ">" + "\n" +
                             embed_message(
                                 f"{own_message}: {filtered_exception}\n"
